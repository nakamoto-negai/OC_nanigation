...
| モデル | 内容 |
|---|---|
| `Node` | 地点。名前・説明・マップ座標(x,y)・GPS座標(lat,lng)・混雑度・待ち時間。中継地点も含む純粋な地図上の点。`show_on_map_select`（*bool・既定true）で「現在地の地図選択」にマーカーとして出すか選べる（中継地点を隠す用途。false を明示保存できるよう *bool） |
| `Destination` | 目的地。ユーザーが選ぶ「行き先」の単位。1つの目的地に複数ノードを多対多(`destination_nodes`)で登録でき、経路案内では現在地から最も近い所属ノードへ案内する。カテゴリ・イベントは目的地に紐づく。is_bus_stop=true の目的地はホームの「バス停選択」地図に出て現在地として選べる。is_stamp_rally=true の目的地は、イベント選択画面でそのイベントカードに「スタンプラリー」ラベルが付く |
| `Category` | 目的地のカテゴリ。目的地選択画面のグループ見出しに使う |
| `Event` | 目的地で開催されるイベント。`destination_id` で目的地に紐づく。`category_id`（任意）でイベント自身をカテゴリーに紐付け、イベント選択画面ではこのカテゴリー（目的地のカテゴリーとは独立）で分類して表示する。`is_stamp_rally`=true でイベント選択に「スタンプラリー対象」ラベルを表示（目的地の is_stamp_rally も併用＝どちらか true でラベル表示）。管理画面「イベント」タブで作成・編集（名前/目的地/カテゴリー/スタンプ）・削除できる |
...
| `Cafeteria` | 食堂。管理画面「食堂」タブから複数登録でき、それぞれ名前・混雑度(0〜4)・並び順・`destination_id`（任意の紐づけ目的地）を持つ。ヘッダーに名前＋混雑度バッジで**ボタン**表示され、目的地が紐づく食堂を押すとその目的地が道案内の行き先に設定される。混雑度は食堂編集用アカウント(/cafeteria)からも更新できる |
